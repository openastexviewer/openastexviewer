...
class CEmit
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(null != spec);
	    CUtility.check(null != outstream);
	  }
  ...
  /*void emit_imports
      ...
      throws java.io.IOException      
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.check(null != m_spec);
	      CUtility.check(null != m_outstream);
	    }*/
  ...
  private void emit_construct
      ...
      throws java.io.IOException
	{
	  ...
	  m_outstream.println();
        ...
        m_outstream.println("\tpublic void setInput(java.io.Reader reader){");
  ...
  private void emit_table
	...
	for (elem=0; elem<size; elem++) {
	    ...
	    CUtility.check(dtrans.m_dtrans.length==m_spec.m_dtrans_ncols);
  ...
  private void emit_actions 
      ...
      throws java.io.IOException
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.check(m_spec.m_accept_vector.size() 
...
class CMakeNfa
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(null != input);
	    CUtility.check(null != lexGen);
	    CUtility.check(null != spec);
	  }
  ...
  void allocate_BOL_EOF
	...
	{
	  CUtility.check(CSpec.NUM_PSEUDO==2);
...
class CMinimize
{
  ...
  private void reduce
	...
	for (reduced_ncols = 0; ; ++reduced_ncols)
	  {
		...
		for (i = 0; i < reduced_ncols; ++i)
		  {
		    CUtility.check(-1 != m_spec.m_col_map[i]);
		  }
	      }
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.check(false == set.get(i));
		CUtility.check(-1 == m_spec.m_col_map[i]);
	      }
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(k == reduced_ncols);
	  }
	...
	for (reduced_nrows = 0; ; ++reduced_nrows)
	  {
		...
		for (i = 0; i < reduced_nrows; ++i)
		  {
		    CUtility.check(-1 != m_spec.m_row_map[i]);
		  }
	      }
	...
	if (CUtility.DEBUG)
	  {
	    ...
	    CUtility.check(k == reduced_nrows);
	  }
      }
	...
	while (old_group_count != group_count)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.check(m_group.size() == group_count);
	      }
		...
		for (j = 1; j < group_size; ++j)
		  {
			...
			if (goto_first != goto_next
				...
				|| m_ingroup[goto_next] != m_ingroup[goto_first]))
			  {
			    if (CUtility.DEBUG)
			      {
				CUtility.check(dtrans_group.elementAt(j) == next);
			      }
			    ...
			    if (CUtility.DEBUG)
			      {
				CUtility.check(m_group.contains(new_group)
						== true);
				CUtility.check(m_group.contains(dtrans_group)
						== true);
				CUtility.check(dtrans_group.contains(first)
						== true);
				CUtility.check(dtrans_group.contains(next)
						== false);
				CUtility.check(new_group.contains(first)
						== false);
				CUtility.check(new_group.contains(next)
						== true);
				CUtility.check(dtrans_group.size() == group_size);
				CUtility.check(i == m_ingroup[first.m_label]);
				CUtility.check((m_group.size() - 1) 
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.check(i == dtrans.m_label);
		CUtility.check(false == group_found);
		CUtility.check(group_count == m_group.size());
	      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.check(false == group_found);
		    CUtility.check(0 < dtrans_group.size());
		  }
		...
		if (CUtility.SLOW_DEBUG)
		  {
			...
			CUtility.check(check.m_accept == first.m_accept);
		      }
		  }
		...
		if (first.m_accept == dtrans.m_accept)
		  {
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.check(j == m_ingroup[dtrans.m_label]);
		      }
...
class CNfa2Dfa
{
	...
	for (istate = 0; nstates > istate; ++istate)
	  {
	    ...
	    */
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.check(false == dfa.m_mark);
		  }
		...
		for (i = 0; i < m_spec.m_dtrans_ncols; ++i)
		  {
		    if (CUtility.DEBUG)
		      {
			CUtility.check(0 <= i);
			CUtility.check(m_spec.m_dtrans_ncols > i);
		      }
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.check((null == bunch.m_nfa_set 
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.check(nextstate < m_spec.m_dfa_states.size());
		      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.check(m_spec.m_dtrans_vector.size() == dfa.m_label);
		  }
  ...
  private void e_closure
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(null != bunch);
	    CUtility.check(null != bunch.m_nfa_set);
	    CUtility.check(null != bunch.m_nfa_bit);
	  }
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.check(bunch.m_nfa_bit.get(state.m_label));
	      }
	...
	while (false == nfa_stack.empty())
	  {
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.check(null != bunch.m_accept);
		    CUtility.check(CSpec.NONE == bunch.m_anchor
		...
		if (null != state.m_next)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.check(false == bunch.m_nfa_bit.get(state.m_next.m_label));
			  }
		...
		if (null != state.m_next2)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.check(false == bunch.m_nfa_bit.get(state.m_next2.m_label));
			  }
...
class CLexGen 
{
  ...
  void generate
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(null != this);
	    CUtility.check(null != m_outstream);
	    CUtility.check(null != m_input);
	    CUtility.check(null != m_tokens);
	    CUtility.check(null != m_spec);
	    CUtility.check(m_init_flag);
	  }
  ...
  private char[] packCode
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(INIT_CODE == specified 
  ...
  private void userRules
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(END_OF_INPUT == m_spec.m_current_token);
	  }
	...
	for (rep_elem = 0; rep_elem < start_macro; ++rep_elem)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.check(rep_elem < replace.length);
	      }
	  }
  ...
  private void saveMacro
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check(0 < count_def);
	    CUtility.check(0 < count_name);
	    CUtility.check(null != m_spec.m_macros);
	  }
  ...
  private void saveStates
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.check('%' == m_input.m_line[0]);
	    CUtility.check('s' == m_input.m_line[1]);
	    CUtility.check(m_input.m_line_index <= m_input.m_line_read);
	    CUtility.check(0 <= m_input.m_line_index);
	    CUtility.check(0 <= m_input.m_line_read);
	  }
  ...
  private void details
	...
	while (names.hasMoreElements())
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.check(null != name);
		CUtility.check(null != def);
	      }
	...
	if (false == m_spec.m_count_chars)
	...
	else
	  {
	    if (CUtility.DEBUG)
	      {
		CUtility.check(m_spec.m_count_lines);
	      }
...
final class SparseBitSet implements Cloneable {
	...
	CUtility.check(size<bits.length);
	insert_block(idx, bnum);
    }
    private void insert_block(int idx, int bnum) {
	CUtility.check(idx<=size);
	CUtility.check(idx==size || offs[idx]!=bnum);
	...
	CUtility.check(!a.get(0) && !a.get(1));
	CUtility.check(!a.get(123329));
	a.set(0); CUtility.check(a.get(0) && !a.get(1));
	a.set(1); CUtility.check(a.get(0) && a.get(1));
	a.clearAll();
	CUtility.check(!a.get(0) && !a.get(1));
	...
	for (int n=0; n<ITER; n++) {
	    ...
	    CUtility.check(a.get(rr) && !a.get(rr+1) && !a.get(rr-1));
	    for (int i=0; i<v.size(); i++)
		CUtility.check(a.get(((Integer)v.elementAt(i)).intValue()));
	}
	SparseBitSet b = (SparseBitSet) a.clone();
	CUtility.check(a.equals(b) && b.equals(a));
	for (int n=0; n<ITER/2; n++) {
	    ...
	    CUtility.check(!b.get(m));
	}
	CUtility.check(!a.equals(b));
	SparseBitSet c = (SparseBitSet) a.clone();
	SparseBitSet d = (SparseBitSet) a.clone();
	c.and(a);
	CUtility.check(c.equals(a) && a.equals(c));
	c.xor(a);
	CUtility.check(!c.equals(a) && c.size()==0);
	d.or(b);
	CUtility.check(d.equals(a) && !b.equals(d));
	d.and(b);
	CUtility.check(!d.equals(a) && b.equals(d));
	d.xor(a);
	CUtility.check(!d.equals(a) && !b.equals(d));
